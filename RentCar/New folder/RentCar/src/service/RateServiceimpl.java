...
public class RateServiceimpl implements IRateService {
	...
	public void addRate(Rates rate) {
		...
		try {
			String sql = "insert into Rates(VechileId,Amount) values (?,?)";
	...
	public ArrayList<Rates> get_Rates_details() {
		...
		try {
			...
			while (resultset.next()) {
				...
				rates.setVechileId(resultset.getString("VechileId"));
	...
	// Update existing Event
		public void updateRates(Rates rates) {
			...
			try {
				...
				String sql = "update Rates set VechileId=?,Amount=? where RateId=? ";
